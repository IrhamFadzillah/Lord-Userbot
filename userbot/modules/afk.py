...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Bentar Gua Afk Dulu**",
    f"**Kintil Sabar Lah Gua Lagi Afk**",
    f"**Kintil Sabar Lah Gua Lagi Afk**",
    f"**Bentar Gua Afk Dulu**",
]
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**/I'm Back Bitch**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**Gua {ALIVE_NAME} Lagi AFK Bentaran Asw** {afk_since} **Yang Lalu.**\
                        \n☛ **Alasan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**Batu Anjir Di Bilang Lagi AFK** {afk_since} **Yang Lalu.**\
                            \n☛ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"**AFK Bentaran** {afk_since} **Yang Lalu**.\
                        \n☛ **Alasan**: `{AFKREASON}`")
                else:
