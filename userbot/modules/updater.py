    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await event.edit(
                ...
                ' untuk dapat deploy perubahan terbaru dari SmokingBot.`'
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await event.edit(
                f'{txt}\n`Kredensial Heroku tidak valid untuk deploy SmokingBot dyno.`'
            )
            return repo.__del__()
        await event.edit('`[HEROKU]:'
                         '\nDyno SmokingBot Sedang Dalam Proses, Mohon Menunggu 7-8 Menit`'
                         )
        ups_rem.fetch(ac_br)
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('** SmokingBot** `Berhasil Di Update!`')
    await asyncio.sleep(1)
    await event.edit('** SmokingBot** `Di Restart....`')
    await asyncio.sleep(1)
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n"
            "**SmokingBot Telah Di Perbarui ツ**")
...
async def upstream(event):
    ...
    if changelog == '' and force_update is False:
        await event.edit(
            f'\n**SmokingBot Sudah Versi Terbaru**\n')
        await asyncio.sleep(15)
        await event.delete()
        return repo.__del__()
...
CMD_HELP.update({
    'update':
    ".update"
    "\nUsage: Untuk Melihat Pembaruan Terbaru SmokingBot."
    "\n\n.update now"
    "\nUsage: Memperbarui SmokingBot."
    "\n\n.update deploy"
    "\nUsage: Memperbarui SmokingBot Dengan Cara Deploy Ulang."
})
